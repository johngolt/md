...
EDA需要达到的目的：变量是否需要筛选、替换和清洗；变量是否需要转换；变量之间是否需要交叉；变量是否需要采样。
...
产生原因：有些信息暂时无法获取，或者获取信息的代价太大、有些信息被遗漏、属性值不存在

产生影响：数据挖掘建模将丢失大量的有用信息；数据挖掘模型所表现出的不确定性更加显著，模型中蕴含的规律更难把握；包含空值的数据会使建模过程陷入混乱，导致不可靠的输出。

缺失值是否被人用某个特殊值填充了，后面需要对缺失进行合理填充。
...
单一值：有时一个特征不提供信息，是因为它拥有太多具备相同值的行。当它们的确无法提供有用信息时，我们就可以丢弃它。
...
不想关：如果特征与项目试图解决的问题无关，则这些特征是不相关数据。

复制：复制数据即，观察值存在副本。复制数据有两个主要类型：基于所有特征、基于关键特征。

###### 不一致数据

1. 大写：在类别值中混用大小写是一种常见的错误。
2. 格式：另一个标准化是数据格式。比如将特征从字符串格式转换为 DateTime 格式。
3. 类别值：分类特征的值数量有限。有时由于拼写错误等原因可能出现其他值。识别它们的一种简单方式是模糊逻辑（或编辑距离）。该方法可以衡量使一个值匹配另一个值需要更改的字母数量（距离）。
