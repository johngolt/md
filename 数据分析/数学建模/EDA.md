...
数据探索可以帮助我们更好地了解数据的性质以及干净程度，包括数据的大小，数据的缺失值的分布，训练集与测试集的分布差异等，同时，数据集中的奇异现象又会进一步促进我们对其进行研究与观察，更好地了解业务，并构建相应强特征；对已有数据在尽量少的先验假设下通过作图、制表、方程拟合、计算特征量等手段探索数据的结构和规律的一种数据分析方法。EDA需要达到的目的：变量是否需要筛选、替换和清洗；变量是否需要转换；变量之间是否需要交叉；变量是否需要采样；
...
- 箱线图：依据实际数据绘制，真实、直观地表现出了数据分布的本来面貌，且没有对数据作任何限制性要求（3σ原则要求数据服从正态分布或近似服从正态分布），其判断异常值的标准以四分位数和四分位距为基础。异常值通常被定义为小于QL-1.5IQR或大于QU+1.5IQR的值。
...
如果特征与项目试图解决的问题无关，则这些特征是不相关数据。

###### 复制

复制数据即，观察值存在副本。复制数据有两个主要类型。基于所有特征、基于关键特征（最好的方法是删除基于一组唯一标识符的复制数据。）
