...
垂直坐标的长度表示每个单词出现的概率$p(x)$，水平坐标的长度表示每个单词的编码长度$L(x)$。如果所有单词的编码长度一样，则面积为2 bit

![](../../picture/2/70.png)

如果常见的单词编码长度更短，不常见的单词编码长度更长，则所有单词所围成的面积小于编码长度相同时围成的面积

![](../../picture/2/71.png)

###### The Space of Codewords

$1$bit有两种编码方式：$0$ and $1$. $2$bit有4种编码方式: $00, 01, 10$, and $11$. $n$bit有$2^n$种编码方式。

![](../../picture/2/72.png)

为了保证编码之后可以进行解码，我们需要我们的codeword满足：no codeword should be the prefix of another codeword。这说明越短的codeword意味着牺牲更多可能的codeword编码。One useful way to think about this is that every codeword requires a sacrifice from the space of possible codewords.

![](../../picture/2/73.png)

###### Optimal Encodings

codeword需要放弃的可能编码空间随着codeword的长度呈指数下降。我们设定需要放弃的编码空间为C，则$C=\frac{1}{2^{L(x)}}$

![](../../picture/2/74.png)

每个单词的平均编码长度我们定义为$p(x)\times L(x)$。如果编码长度$L(x)$越小，则整体的平均长度越小，但是$C$越大；如果编码长度$L(x)$越大，则整体的平均长度越大，但是$C$越小。

![](../../picture/2/75.png)

假设事件$a$发生的概率为$p(a)$，事件$b$发生的概率为$p(b)$。我们使得$a$编码的$C$为$p(a)$，$b$编码的$C$为$p(b)$。我们知道$a$的benefit为$f(x)=p(a)\times L(x)$，a的cost为$C=2^{-L(x)}$

![](../../picture/2/76.png)

当$C=p(a)$，我们有$\frac{f^{\prime}}{C^{\prime}}=\frac{p(a)}{-p(a)\ln2}=-\frac{1}{\ln2}$。与事件$a$的发生概率无关，所有对于事件$b$也满足，则在此时候，带来的好处和代价对于所有的事件都是一样的。

![](../../picture/2/77.png)

 We’ll invest $ϵ$ less in $b$, and invest it in $a$ instead. This makes the codeword for $a$ a bit shorter, and the codeword for $b$ a bit longer. Now the cost of buying a shorter codeword for $a$ is $p(a)+ϵ$, and the cost of buying a shorter codeword for $b$ is $p(b)−ϵ$. But the benefits are still the same. This leads the benefit cost ratio for buying $a$ to be $\frac{p(a)}{p(a)+ϵ}$ which is less than one. On the other hand, the benefit cost ratio of buying $b$ is $\frac{p(b)}{p(b)−ϵ}$ which is greater than one.

![](../../picture/2/78.png)

The more different the distributions $p$ and $q$ are, the more the cross-entropy of $p$ with respect to $q$ will be bigger than the entropy of $p$.We call this difference the KL divergence, or just the KL divergence. The KL divergence of $p$ with respect to $q$, $D_{q}(p)$.
...
V(X, Y)=H(X, Y)-I(X, Y)
$$
![](../../../学习/UsedMD/picture/2/81.png)

![](../../../学习/UsedMD/picture/2/82.png)
$$
\begin{array}{l}
\text{自信息}：I(x)=-\log P(x)\\
